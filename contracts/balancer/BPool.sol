...
contract BPoolBase is BBronze, BToken, BMath {
  ...
  // Target balance for a token which does not yet have a sufficient
  // balance to be traded
    ...
    returns (uint256 tokenAmountOut, uint256 spotPriceAfter)
  {
    ...
    require(_targetBalances[tokenOut] == 0, "ERR_TOKEN_NOT_READY");

    Record memory inRecord = _records[address(tokenIn)];
    Record memory outRecord = _records[address(tokenOut)];
    uint256 targetBalance = _targetBalances[tokenIn];
    if (targetBalance == 0) {
      require(
  ...
  /**
   * @dev Calculates the new weight for a token.
   * This will not allow the weight to be adjusted by more than
   * the maximum weight change, which is calculated as
   * (totalWeight * swapFee / 2)
  */
  function _calcNewWeight(
    uint96 desiredDenorm,
    uint96 denorm
  ) internal view returns (uint96) {
    bool incr = desiredDenorm > denorm;
    uint96 maxDiff = uint96(bmul(MAX_WEIGHT_CHANGE, _totalWeight));
    uint96 realDiff = incr
      ? uint96(bsub(desiredDenorm, denorm))
      : uint96(bsub(denorm, desiredDenorm));
    // difference is allowable
    if (maxDiff > realDiff) return desiredDenorm;
    // difference exceeds the max difference
    return incr
      ? uint96(badd(denorm, maxDiff))
      : uint96(bsub(denorm, maxDiff));
  }
}
