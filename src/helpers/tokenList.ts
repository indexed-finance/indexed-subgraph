...
export function fetchTokenFromTokenList(tokenAddress: Address) : Token | null {
  ...
  for (let i: i32 = 0, len: i32 = tokens.length; i < len; i++) {
    ...
    if(tokenData.isSet('decimals') && tokenData.get('decimals').kind == JSONValueKind.NUMBER) {
      token.decimals = tokenData.get('decimals').toI64() as i32
      log.info('Token decimals found in Token List', [tokenAddress.toHexString(), BigInt.fromI32(token.decimals).toString()])
    }
    return token
  }
...
export function fetchTokenDecimalsFromTokenList(tokenAddress: Address): i32 {
  let token = fetchTokenFromTokenList(tokenAddress)
  if(token != null) {
